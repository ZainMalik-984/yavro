...
const SpinnerManagement: React.FC = () => {
  ...
  const [editingOption, setEditingOption] = useState<SpinnerOption | null>(
    null
  );
  ...
  const loadRewards = async () => {
    try {
      ...
      const spinnerRewards = rewardsData.filter(
        reward => reward.reward_type === 'spinner'
      );
  ...
  const handleDelete = async (optionId: number) => {
    if (
      window.confirm('Are you sure you want to delete this spinner option?')
    ) {
      try {
    ...
    return (
      <div className='spinner-management'>
        ...
        <div className='no-spinner-rewards'>
          <p>
            No spinner rewards found. Create a reward with type "Spinner" first!
          </p>
        </div>
      </div>
    );
    ...
    <div className='spinner-management'>
      ...
      {showForm && (
        <div className='form-overlay'>
          <div className='form-container'>
            <h3>
              {editingOption ? 'Edit Spinner Option' : 'Add New Spinner Option'}
            </h3>
      ...
      <div className='spinner-options-list'>
        ...
        {spinnerOptions.length === 0 ? (
          <p className='no-data'>
            No spinner options found. Add your first option!
          </p>
        ) : (
            ...
            {spinnerOptions.map(option => (
              <div key={option.id} className='spinner-option-card'>
                <div className='option-header'>
                  <h4>{option.name}</h4>
                  <span
                    className={`status ${option.is_active ? 'active' : 'inactive'}`}
                  >
                ...
                <div className='option-details'>
                  <p>
                    <strong>Type:</strong>{' '}
                    {getRewardTypeLabel(option.reward_type)}
                  </p>
                  {option.value && (
                    <p>
                  ...
                  <p>
                    <strong>Probability:</strong> {option.probability}
                  </p>
                  {option.description && (
                    <p>
