...
def get_user_by_face_encoding(db: Session, encoding: np.ndarray, tolerance: float = 0.4):
    ...
    for user in users:
        try:
            ...
            logging.info(f"DB encoding shape for user {user.id}: {db_encoding.shape}")
            ...
            matches = face_recognition.compare_faces([db_encoding], encoding, tolerance=tolerance)
            ...
            if matches[0]:
                ...
                logging.info(f"No match for user {user.id} ({user.name}): distance={distance}")
...
def update_spinner_option(db: Session, option_id: int, option: schemas.SpinnerOptionCreate):
    db_option = db.query(models.SpinnerOption).filter(models.SpinnerOption.id == option_id).first()
    if db_option:
...
def check_tier_upgrade(db: Session, user_id: int):
    ...
    tiers = db.query(models.Tier).filter(models.Tier.is_active == True).order_by(models.Tier.visit_requirement).all()
