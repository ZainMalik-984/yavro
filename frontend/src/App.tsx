...
import { User, Token, AdminUser } from './types';
...
import AppAdministration from './components/AppAdministration';
...
function AppContent() {
  ...
  const [showAppAdministration, setShowAppAdministration] = useState(false);
  ...
  const handleLoginSuccess = (token: Token) => {
    setCurrentAdminUser(token.user);
    setIsAuthenticated(true);
  };

  const handleShowSuperAdminRegistration = () => {
    setShowSuperAdminRegistration(true);
  };

  const handleSuperAdminRegistrationSuccess = () => {
  ...
  const handleLogout = () => {
    ...
    setShowAppAdministration(false);
  };
  ...
  const handlePhoneSearchComplete = (user: User) => {
    setCurrentUser(user);
    setShowPhoneSearch(false);
    setShowCheckout(true);
  };

  const handlePhoneSearchNotFound = () => {
    setShowPhoneSearch(false);
    setShowRegistration(true);
  };
  ...
  // If not authenticated, show login or super admin registration
  if (!isAuthenticated) {
    if (showSuperAdminRegistration) {
      return (
        <SuperAdminRegistration
          onRegistrationSuccess={handleSuperAdminRegistrationSuccess}
    ...
    return (
      <Login 
        ...
        onShowSuperAdminRegistration={handleShowSuperAdminRegistration}
      />
    );
  }
    ...
    <div className='App'>
          ...
          <div className='header-actions'>
            {currentAdminUser && (
              <div className='user-info'>
                <span className='user-name'>{currentAdminUser.username}</span>
                <span className={`user-role ${currentAdminUser.role}`}>
                  {currentAdminUser.role.toUpperCase()}
                </span>
              </div>
            )}
            <button
            ...
            {(currentAdminUser?.role === 'admin' || currentAdminUser?.role === 'super-admin') && (
              <button
                className='admin-button'
                onClick={() => setShowAppAdministration(true)}
      ...
      <main className='App-main'>
        {showDashboard ? (
          ...
          <>
            {!showRegistration && !showPhoneSearch && !showCheckout && (
              <CameraCapture
            ...
            {showPhoneSearch && (
              <PhoneSearch
                onUserFound={handlePhoneSearchComplete}
                onUserNotFound={handlePhoneSearchNotFound}
                onBack={handleReset}
                isLoading={isLoading}
                setIsLoading={setIsLoading}
              />
        ...
        {/* App Administration Modal */}
        {showAppAdministration && (
          <AppAdministration
            onClose={() => setShowAppAdministration(false)}
          />
        )}
